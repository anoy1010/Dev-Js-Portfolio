...
import { getSearches, getAvatar, firebaseAuth } from '../firebase';
...
const ItemReview = ({ item }) => (
  <View style={{ flex: 1, flexDirection: 'row', marginBottom: 10, marginTop: 10 }} >
    ...
    <View style={{ flex: 3 }}>
      <Text style={{ fontSize: 18, color: '#4F4F4F' }}>{item.post}</Text>
    </View>
    <View style={{ flex: 1 }}>
...
export default function MySearches({ navigation }) {
  ...
  const fetchUser = async () => {
    ...
    getSearches(firebaseAuth.currentUser.uid).then((data) => {
